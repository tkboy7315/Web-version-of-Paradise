...
(function () {
  ...
  // 直接在首頁 #main-menu 插入「📥 安裝成 APP」按鈕
  function injectInstallButton() {
    if (isStandalone()) return; // 已安裝就不顯示
    var menu = document.getElementById('main-menu');
    if (!menu || document.getElementById('afk-pwa-install-btn')) return;
    var btn = document.createElement('button');
    btn.id = 'afk-pwa-install-btn';
    btn.type = 'button';
    btn.textContent = '📥 安裝成 APP';
    btn.style.cssText = 'background:linear-gradient(135deg,#1e3a5f 0%,#2563eb 50%,#1e3a5f 100%);color:#bfdbfe;border:1px solid #3b82f6;border-radius:8px;padding:8px 18px;font-size:14px;font-weight:bold;cursor:pointer;width:100%;margin-top:-2px;';
