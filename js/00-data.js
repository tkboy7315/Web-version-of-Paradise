...
const GAME_VERSION = 'v3.8.34-modified';   // 🏷️ 版本號：末段 0~99 線性遞增，達 100 進位（中位 +1、末段歸 0）
...
"wpn_giltas_wand":   { n: "吉爾塔斯魔杖", type: "wpn", w2h: true, legend: true, dmgS: 15, dmgL: 15, hit: 0, dmgBonus: 0, spd: 1.0, req: "mage,illusion", safe: 0, p: 990000, gachaWeight: 0, eff: "magicburst", int: 2, wis: 2, mpR: 15, d: "吉爾塔斯魔力凝成的雙手魔杖，杖端迴盪著異界的低語。魔爆（一般限定）、貫穿、智力+2、精神+2、MP自然恢復量+15；擊殺敵人後，依邪惡值提高額外魔法點數，滿邪惡時額外魔法點數+20，持續10秒。" },
    ...
    towns: {
        ...
        "town_sherine": {   // 🔮 新安全區：席琳神殿
            n: "席琳神殿",
            npcs: [
                { id: "npc_sherine", n: "席琳", title: "祈禱", type: "pray", d: "靜謐的神女席琳，傾聽虔誠者的禱詞。等級 40 以上可向席琳祈禱，開啟或關閉「席琳的世界」。開啟期間，席琳世界的怪物（含頭目）被擊殺時有 5% 機率觸發「即死恢復」——不死並將血量回滿（每隻僅觸發一次）。" },
...
function _origEnforce() {
  try {
    if (window._origBannerDismissed) return;
    ...
    setTimeout(function() { bar.style.opacity = '0'; window._origBannerDismissed = true; }, 5000);
    setTimeout(function() { if (bar.parentNode) bar.parentNode.removeChild(bar); }, 5800);
  } catch (_) {}
}
...
// 🎯 經驗倍率函數（×1/×2/×3 下拉選單用）
function getExpBonusMult() {
    let m = (typeof state !== 'undefined' && state) ? (state.expMult || 1) : 1;
    if (m <= 1) return 1;
    let isClassic = (typeof player !== 'undefined' && player && player.classicMode);
    if (m === 2) return isClassic ? 3 : 10;
    if (m === 3) return isClassic ? 5 : 20;
    return 1;
}
// 🎯 金幣倍率函數（×1/×2/×3 下拉選單用）
function getGoldBonusMult() {
    let m = (typeof state !== 'undefined' && state) ? (state.goldMult || 1) : 1;
    if (m <= 1) return 1;
    let isClassic = (typeof player !== 'undefined' && player && player.classicMode);
    if (m === 2) return isClassic ? 3 : 5;
    if (m === 3) return isClassic ? 5 : 10;
    return 1;
}
// 🎯 物品掉落倍率函數（×1/×2/×3 下拉選單用）
function getDropBonusMult() {
    let m = (typeof state !== 'undefined' && state) ? (state.dropMult || 1) : 1;
    if (m <= 1) return 1;
    let isClassic = (typeof player !== 'undefined' && player && player.classicMode);
    if (m === 2) return isClassic ? 2 : 3;
    if (m === 3) return isClassic ? 3 : 5;
    return 1;
}
