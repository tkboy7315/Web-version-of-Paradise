...
function gameLoop() {
    ...
    _tickDebt += (_hidden ? elapsed : Math.min(elapsed, FF_MAX_ELAPSED_MS)) * (state.spd || 1);   // 背景凍結可能超過 5 分鐘：全額保留在債務中，不能因單次上限遺失收益；state.spd 加速倍率
    ...
    if (state.spd > 1 && owed > 1 && !_hidden && !_ffAcc) {
        ...
        while (_tickDebt >= TICK_MS && state.running && !player.dead) {
            _tickDebt -= TICK_MS;
            state.inTick = true;
            try { tick(); } finally { state.inTick = false; settleDeadMobs(); }
        }
