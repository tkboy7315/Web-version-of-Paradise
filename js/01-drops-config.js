...
//  +0~+10 維持原本「每階 +1」線性；+11 起在「+10 的量」之上再加累加公式。
...
//    一律以淬鍊上限計算——武器/防具/飾品超過 +50 以 +50 計。
...
//  +11~+50：取消額外傷害加成（額外傷害維持在 +10 的量＝10）；額外命中改用累加公式：+11 起每階 floor(i/3)+1；
//           傷害成長改由「最終傷害倍率」提供（enhanceWpnFinalMult，非累加、取該階段數值：+11 起每階 +0.1）。
function enhanceWpnBonus(en) {
    en = Math.max(0, Number(en) || 0);
    let base = Math.min(en, 10);                                                            // +10 以內：每階 +1（額外傷害/命中共用）
    let hitOver = 0;
    for (let i = 11; i <= Math.min(en, 50); i++) hitOver += Math.floor(i / 3) + 1;           // +11~+50：累加公式每階 floor(i/3)+1
    return { dmg: base, hit: base + hitOver };                                              // 額外傷害封頂於 10（+11 起不再加）
}
// 武器強化 → 最終傷害倍率（一般物理攻擊）；+11~+50 每階 +0.1，+10 以內為 1.0
function enhanceWpnFinalMult(en) {
    en = Math.max(0, Number(en) || 0);
    return (en > 10) ? (1 + (Math.min(en, 50) - 10) * 0.1) : 1;
}
function wpnEnFinalMult(wpnInst) { return enhanceWpnFinalMult(wpnInst && wpnInst.en); }      // 由武器實例取倍率（未裝備→1）
// 防具強化 → AC 減免量（值越大防禦越好）；+11~+50 每階 AC +2
function enhanceArmAc(en) {
    en = Math.max(0, Number(en) || 0);
    let base = Math.min(en, 10);
    let over = (en > 10) ? (Math.min(en, 50) - 10) * 2 : 0;
    return base + over;
}
