...
function classicDropMult() { return player.classicMode ? 0.1 : 10; }   // 🎮 經典模式：所有物品掉落機率 ×1/10；一般模式 ×10
...
function killMob(idx) {
    ...
    player.exp += Math.floor(mob.exp * getExpGainMult(player.lv) * (player.classicMode ? 0.5 : 100));   // 🎮 經典模式：經驗值減半；一般模式 ×100
    checkLvUp();
    ...
    if (!_kbNoReward && Math.random() < 0.8) {
        ...
        if (player.classicMode) g = Math.floor(g / 2); else g *= 50;   // 🎮 經典模式：怪物金幣僅剩一般模式的 1/2；一般模式 ×50
        player.gold += g;
