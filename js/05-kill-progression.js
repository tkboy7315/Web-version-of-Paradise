...
function classicDropMult() { return RATE_PRESETS[state.rateLv] ? RATE_PRESETS[state.rateLv].drop : 1; }
...
function killMob(idx) {
    ...
    player.exp += Math.floor(_expShare * getExpGainMult(player.lv) * (1 + dollFieldVal('expBonus') / 100) * (RATE_PRESETS[state.rateLv] ? RATE_PRESETS[state.rateLv].exp : 1));   // ⚠️v3.0.82 經典×0.5 已移除
    checkLvUp();
    ...
    if (!_kbNoReward && Math.random() < 0.8) {
        ...
        g = Math.floor(g * (1 + dollFieldVal('goldBonus') / 100) * (RATE_PRESETS[state.rateLv] ? RATE_PRESETS[state.rateLv].gold : 1));   // 🪆 魔法娃娃 goldBonus%（莫提斯）
        player.gold += g;
