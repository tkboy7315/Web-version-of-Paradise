...
(function () {
  ...
  var WEAPON_TRAITS = [
    ...
    { n: '束縛', d: '攻擊命中時有機率使目標「束縛」：無法移動數秒，但仍可攻擊。代表武器：深紅之弩等帶 procStatus: bind 的武器。' },
    { n: '即死（遺物）', d: '多種遺物提供不同形式的即死機制：骰子匕首 1%、強力股骨 20% 限定不死類、陰影刺擊 HP低於10% 即死、碎擊之錘 1% 限定硬皮怪等。對 BOSS 無效。' },
    { n: '觸發技能（遺物）', d: '命中時自動施放指定技能（不消耗MP）。觸發機率與冷卻各遺物不同。代表遺物：雷霆法杖→雷霆、熔岩噴射→火焰魔法、地獄犬杖→噴火、大地崩塌杖→地震等。走攻擊魔法傷害公式。' },
    { n: '元素爆破（遺物）', d: '命中後追加等同本次一般攻擊傷害的指定屬性魔法傷害（機率觸發）。代表遺物：烈焰爆破杖（火屬性）、海嘯之弓（水屬性）等。' },
    { n: '屬性附加傷害（遺物）', d: '命中時附加固定屬性傷害。代表遺物：火焰戰士劍（火+50）、冰晶鎚（水+10）、風刃拳套（風+3%觸發）等。' },
    { n: '雙擊（遺物）', d: '命中時有機率造成雙倍傷害（可與爆擊/重擊同時觸發）。代表遺物：艾爾摩巨劍 3%。' },
    { n: '屠龍一擊（遺物）', d: '對龍族造成額外巨額傷害。代表遺物：真·屠龍劍 15%觸發5D60+60，龍族×3。' },
    { n: '連殺（遺物）', d: '每次命中增加連殺計數，達到指定次數後觸發額外效果（如全體傷害/隨機效果）。代表遺物：冰冷蒼焰。' },
    { n: '創傷（遺物）', d: '命中時有機率疊加創傷層數，每層持續造成傷害（可疊加）。代表遺物：戰士黑刃 5%觸發，最多2層，每層5點/秒，持續6秒。' },
    { n: '審判落雷（遺物）', d: '觸發時造成風屬性魔法傷害並麻痺目標。需裝備指定武器才能觸發。代表遺物：克特盾（需配克特劍）。' },
    { n: '碎擊龍捲風（遺物）', d: '重擊時觸發龍捲風造成全體傷害。代表遺物：長者黑曜石。' },
    { n: '寒霜吐息（遺物）', d: '攻擊時有機率冰凍敵人。代表遺物：冰死吐息。' },
    { n: '火焰牢獄（遺物）', d: '對灼燒中的敵人傷害倍增。代表遺物：火焰皮甲 ×2倍。' },
    { n: '硬皮引燃（遺物）', d: '攻擊硬皮敵人時引燃目標，造成持續火焰傷害。代表遺物：地狱火鎚。' },
    { n: '自動施法反噬（遺物）', d: '自動施法失敗時反噬自身（造成傷害）。代表遺物：血儀匕首。' },
    { n: '無視魔防（遺物）', d: '魔法傷害無視目標MR。代表遺物：血儀匕首。' }
  ];
  var WEAPON_BASICS = [
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '怪打你時：迴避(ER)・傷害減免(DR)・受到傷害減少%', blocks: [
      ...
      { t: 'tbl', h: ['防禦層', '作用', '來源／規則'], rows: [
        ['① 迴避率（ER）', '有機率<b>整個閃掉（0 傷害）</b>', '主要靠<b>敏捷</b>＋增益/套裝（迴避提升+12、月光3件+5）。<b>非 1:1</b>：50 以內 1 點=1%、超過 50 每 5 點才+1%。一般只閃物理'],
        ['② 傷害減免（DR）', '沒閃掉時<b>固定扣掉 DR</b>（物理魔法都扣）', '套裝（鐵衛2件+5、紅獅3件+10）、龍之護鎧+5、巴風特盔甲+2、守護者臂甲、耐力、遠古/太初防具'],
        ['③ 受到傷害減少 %', '扣完 DR 後<b>整體乘算</b>（各效果相乘、不是加總）', '鐵衛3件＝×0.8（−20%）、幻覺化身 −10%、龍裔（龍血3件觸發）−15%、狂怒5件依失血最多 −15%（HP≤50%達上限）']
      ]},
    ...
    { t: '💀 骷髏復生（死靈之書遺物特效）', blocks: [
      { t: 'tbl', h: ['項目', '內容'], rows: [
        ['觸發條件', '裝備「死靈之書」（法師限定盾牌欄遺物）'],
        ['自動召喚', '任意敵人被擊敗時，自動召喚 1 隻骷髏（不消耗 MP）'],
        ['骷髏上限', '全隊場上最多 <b>6 隻</b>（含玩家＋傭兵＋召喚物共用上限）'],
        ['上限時效果', '已達 6 隻時，擊殺敵人改為<b>完全恢復 HP 最低的骷髏</b>'],
        ['骷髏等級分階', '依玩家等級自動調整：Lv.24-30 → 骷髏戰士、Lv.31-40 → 骷髏騎士、Lv.41-50 → 骷髏法師、Lv.51-60 → 骷髏將軍、Lv.61-70 → 骷髏王者、Lv.71+ → 骷髏帝王'],
        ['攻擊方式', '近戰物理攻擊（ASPD=10 ticks，約每秒 1 次）'],
        ['HP 縮放', '骷髏 HP 依等級分階成長（與玩家等級掛鉤）'],
        ['與召喚術互斥', '骷髏復生的骷髏<b>不占用「召喚術」的召喚上限</b>，兩套系統獨立運作'],
        ['亡者餽贈', '擊殺敵人時，全體玩家、傭兵、召喚物、寵物與護衛恢復 1% 最大 HP']
      ]},
      { t: 'p', p: '骷髏復生的骷髏是「召喚物」但獨立於召喚術系統。骷髏會自動索敵攻擊，死亡後下次擊殺敵人時會自動補回。造屍術（骷髏復生）改為不消耗 MP，法師可專注於其他攻擊魔法。' }
    ]},
    { t: '⚔ 經典模式（硬核挑戰）', lines: [
    ...
    { t: '🏺 遺物系統總覽', blocks: [
      { t: 'p', p: '<b>遺物</b>是特殊的裝備（金色邊框），與一般裝備的差異：①<b>不可強化</b>（noEnhance）②<b>不參與套裝效果</b>③<b>不進強化計算</b>④掉落時<b>必定稀有</b>（gachaWeight=0）⑤每件遺物有<b>獨特的特效</b>（下面分類說明）。遺物可從怪物掉落、潘朵拉黑市、製作取得，「遺物尋寶」特效可使掉落判定次數×2。' },
      { t: 'tbl', h: ['分類', '部位', '數量', '核心定位'], rows: [
        ['⚔️ 武器遺物', '單手劍/雙手劍/矛/鈍器/弓/魔杖/鋼爪/雙刀/鎖鏈劍/武士刀等', '~155 件', '攻擊特效（即死/觸發技能/屬性傷害/連擊增幅）'],
        ['🛡️ 防具遺物', '頭盔/盔甲/脛甲/斗篷/手套/鞋子/盾牌/臂甲/內衣', '~165 件', '防禦特效（免疫異常/荆棘反傷/傷害反射/屬性化身）'],
        ['💍 飾品遺物', '項鍊/戒指/腰帶/耳環/寵物裝備', '~45 件', '輔助特效（寵物強化/召喚強化/回復增幅/特殊被動）']
      ]},
      { t: 'p', p: '遺物的特效在<b>裝備時自動生效</b>（無需主動開啟），且多數遺物特效<b>在經典模式仍生效</b>（與一般武器特效不同）。' }
    ]},
    { t: '🏺 遺物特效分類：攻擊型', blocks: [
      { t: 'tbl', h: ['特效名稱', '效果說明', '代表遺物'], rows: [
        ['即死系', '命中時有機率直接擊殺目標（對BOSS無效）', '骰子匕首 1%、強力股骨 20%（限不死類）、陰影刺擊 HP<10%、碎擊之錘 1%（限硬皮怪）'],
        ['觸發技能', '命中時自動施放指定技能（不耗MP）', '雷霆法杖→雷霆、熔岩噴射→火焰魔法、地狱犬杖→噴火等'],
        ['屬性附加傷害', '命中時附加指定屬性固定傷害', '火焰戰士劍→火傷害、冰晶鎚→水傷害、風刃拳套→風傷害'],
        ['元素爆破', '命中後追加等同本次傷害的屬性魔法傷害', '烈焰爆破杖（火）、海嘯之弓（水）等'],
        ['重擊增幅', '重擊傷害倍增或重擊率提升', '狂戰士斧（重擊×2）、地裂劍（重擊率+）'],
        ['雙擊', '命中時有機率造成雙倍傷害', '艾爾摩巨劍 3%'],
        ['屠龍一擊', '對龍族造成額外巨額傷害', '真·屠龍劍 15%觸發5D60+60，龍族×3'],
        ['出血/中毒', '命中時使目標持續掉血', '毒蛇之牙（中毒）、血色雙刀（出血）'],
        ['石化斬殺', '命中石化中的敵人時即死', '美杜莎刺針'],
        ['滿血斬殺', '命中滿血敵人時即死（一定比例）', '死神鐮刀 100%滿血即死']
      ]},
      { t: 'p', p: '攻擊型遺物的觸發機率各不同（1%~100%），多數為「攻擊時」觸發（含一般攻擊與技能命中）。部分遺物有冷卻時間或特殊條件限制。' }
    ]},
    { t: '🏺 遺物特效分類：防禦型', blocks: [
      { t: 'tbl', h: ['特效名稱', '效果說明', '代表遺物'], rows: [
        ['免疫異常', '免疫石化/中毒/麻痺/灼燒/冰凍中的一種或多种', '各種盾牌/頭盔/盔甲遺物'],
        ['荊棘反傷', '受擊時反彈固定傷害', '荊棘盾（反彈N點傷害）'],
        ['傷害反射', '受擊時有機率完全迴避並反射傷害', '鏡之盾 10%反射'],
        ['屬性化身', '自身轉為指定屬性，承受傷害套用屬性剋制', '天空之神化身（風）、熔岩之心（火）等'],
        ['硬皮防護', '自身獲得硬皮特性，減少物理傷害', '守護者謎語'],
        ['石化精華', '被攻擊時有機率使攻擊者石化', '石化精華'],
        ['亡靈庇護', '每N秒可免疫一次死亡', '十字墓盾 5秒CD'],
        ['受擊爆裂', '受擊時對全體敵人造成火焰魔法傷害', '火焰護甲'],
        ['受擊反制', '受擊時立即觸發一次連射', '荊棘之弓'],
        ['物理防護', '一般攻擊傷害減免（每3秒一次）', '重甲遺物']
      ]}
    ]},
    { t: '🏺 遺物特效分類：輔助型', blocks: [
      { t: 'tbl', h: ['特效名稱', '效果說明', '代表遺物'], rows: [
        ['常駐隱身', '不主動吸引一般怪物', '隱身斗篷'],
        ['低仇恨', '被攻擊權重必定為1（最低）', '隱身斗篷'],
        ['快速再生', 'HP自然恢復間隔縮短', '再生斗篷'],
        ['滿血狙擊', '對滿血敵人一般攻擊傷害倍增', '精準之弓×3'],
        ['滿血施法', '自身滿血時魔力消耗減半', '智者法杖'],
        ['加速突擊', '加速時命中與傷害+30', '疾風之劍'],
        ['擦傷補正', '未命中時改判為擦傷造成50%傷害', '命運之戒'],
        ['賜予技能', '裝備時自動學會指定技能', '魔法筆記（冰錐術）'],
        ['許願戒指', '穿戴時隨機獲得一種能力加成', '精靈之戒'],
        ['追蹤強化', '指定怪物出現率由50%提高至70%', '追蹤者腰帶']
      ]},
      { t: 'p', p: '輔助型遺物多為<b>常駐被動效果</b>，裝備後永久生效，無觸發機率或冷卻限制。' }
    ]},
    { t: '🏺 遺物特效分類：寵物/召喚型', blocks: [
      { t: 'tbl', h: ['特效名稱', '效果說明', '代表遺物'], rows: [
        ['寵物傷害/命中', '提升寵物的傷害與命中值', '惡魔之牙（傷害+3、命中+9）'],
        ['寵物防禦', '提升寵物的防禦值', '馴獸師臂甲（防禦+5）'],
        ['全寵物強化', '所有寵物的魔法傷害/MP恢復/HP提升', '蜥蜴王冠（魔法+3、MP回+5、HP+100）'],
        ['寵物出血', '寵物攻擊命中時附加出血', '血牙項鍊'],
        ['寵物護甲', '寵物受到傷害百分比減免', '守護者項鍊'],
        ['寵物復活強化', '寵物死亡時立即復活並獲得短暫無敵', '寵物虔誠'],
        ['全誘餌', '寵物可使用所有誘餌技能', '馴獸師飼料袋'],
        ['召喚傷害/命中', '提升召喚物的傷害與命中值', '亡者之書（傷害+召喚物+X）'],
        ['召喚物魔法傷害', '提升召喚物的魔法傷害', '珍貴胡蘿蔔'],
        ['骷髏復生', '敵人被擊敗時自動召喚骷髏（全隊最多6隻）', '死靈之書（法師限定）'],
        ['亡者餽贈', '擊殺敵人時全隊恢復HP', '死靈之書（1%最大HP）']
      ]},
      { t: 'p', p: '寵物/召喚型遺物的效果會隨召喚物數量增加而更有價值。骷髏復生的骷髏是獨立於召喚術系統的特殊召喚物。' }
    ]},
    { t: '🏺 遺物特效分類：特殊機制', blocks: [
      { t: 'tbl', h: ['特效名稱', '效果說明', '代表遺物'], rows: [
        ['變身烈焰死騎', '裝備時變身為烈焰的死亡騎士', '烈焰死亡騎士之劍'],
        ['屬性反擊', '受到指定屬性傷害時反擊', '未央劍（地/風）、茶壺錘（地/火）'],
        ['審判落雷', '觸發時造成風屬性傷害並麻痺', '克特盾（需配克特劍）'],
        ['碎擊龍捲風', '重擊時觸發龍捲風全體傷害', '長者黑曜石'],
        ['連殺', '每次命中增加連殺計數，N次後觸發額外效果', '冰冷蒼焰'],
        ['創傷', '命中時疊加創傷層數，每層持續掉血', '戰士黑刃'],
        ['火焰牢獄', '對灼燒中的敵人傷害倍增', '火焰皮甲×2'],
        ['暴怒加速', '爆擊時有機率攻速提升', '鋸齒之牙'],
        ['連擊必爆', '連擊時必定爆擊', '鋸齒之牙'],
        ['閃避轉減傷', 'DR = 閃避(ER) ÷ N', '斯芬克斯黑翼（÷5）'],
        ['寒霜吐息', '攻擊時有機率冰凍敵人', '冰死吐息'],
        ['變身全屬性', '變身時六維全+N', '千面化身+2'],
        ['能力值陣列', '六維屬性依等級成長', '魔力陣列靴'],
        ['無視魔防', '魔法傷害無視目標MR', '血儀匕首'],
        ['自動施法反噬', '自動施法失敗時反噬自身', '血儀匕首']
      ]}
    ]}
  ];
  ...
  function renderMap() {
    ...
    // 🐉 侵蝕安塔瑞斯巢穴：不進 MAP_REGIONS 的副本地圖，這裡補充說明
    var antharasMaps = ['antharas_nest_1', 'antharas_nest_2', 'antharas_nest_3', 'antharas_lair'];
    var antharasNames = { antharas_nest_1: '安塔瑞斯巢穴 第一區', antharas_nest_2: '安塔瑞斯巢穴 第二區', antharas_nest_3: '安塔瑞斯巢穴 第三區', antharas_lair: '安塔瑞斯巢穴 深處（BOSS）' };
    if (antharasMaps.some(function (m) { return DB.maps && DB.maps[m]; })) {
      h += '<div class="m-wiki-sub">🐉 侵蝕安塔瑞斯巢穴（每日副本）</div>';
      h += '<div class="m-wiki-note" style="margin-top:0;"><b>每角色每日限入 1 次</b>（跨角色獨立計算）。4 區推進：第 1→2→3 區打小怪，第 4 區（深處）打 BOSS 安塔瑞斯（三階段變身：被侵蝕→狂怒→瘋狂）。各區內不能回城、死亡會被傳回巢穴入口。組隊時隊員需同區才能一起打；可邀請其他玩家「助戰」（每人每天限助戰 1 次）。詳見「血盟」分頁的安塔瑞斯章節。</div>';
      antharasMaps.forEach(function (m) {
        if (!DB.maps || !DB.maps[m]) return;
        var lv = mapLvRange(m), bits = [];
        bits.push('<span class="c-mappath">📍 每日副本（每角色每日限入 1 次）</span>');
        if (lv) bits.push('<b style="color:#86efac;margin:0;">' + lv + '</b>');
        h += '<div class="m-wiki-kv"><b>' + wDexLink(antharasNames[m] || m) + '</b>' + bits.join('　') + '</div>';
      });
    }
    return h;
  }
  ...
  var EQUIP_GROUPS = (typeof EQUIP_CATEGORIES !== 'undefined' ? EQUIP_CATEGORIES.filter(function (c) { return c.group === '武器'; }).map(function (c) { return { k: c.key, n: '⚔️ ' + c.name }; }) : [{ k: 'wpn', n: '⚔️ 武器' }]).concat([
    ...
    { k: 'boots', n: '🥾 鞋子' }, { k: 'shin', n: '🦵 脛甲' }, { k: 'belt', n: '🎗️ 腰帶' }, { k: 'ring', n: '💍 戒指' },
  ...
  function equipCompact(d) {
    ...
    if (d.relic) {
      ...
      if (d.procStatusSkill) _relicTags.push('異常觸發');
      if (d.procSkill && typeof d.procSkill === 'string') _relicTags.push('觸發技能');
      if (d.spellProc) _relicTags.push('攻擊附加魔法');
      if (d.procInstakill) _relicTags.push('即死');
      if (d.procPoison) _relicTags.push('附毒');
      if (d.procBurn) _relicTags.push('灼燒');
      if (d.onHitEleDmg) _relicTags.push('屬性附加');
      if (d.vampPct) _relicTags.push('吸血');
      if (d.flameDkMorph) _relicTags.push('變身死騎');
      if (d.counterEles) _relicTags.push('屬性反擊');
      if (d.judgmentThunder) _relicTags.push('審判落雷');
      if (d.crushTornado) _relicTags.push('碎擊龍捲風');
      if (d.dblStrikeRate) _relicTags.push('雙擊');
      if (d.dragonSlayStrike) _relicTags.push('屠龍一擊');
      if (d.slaughterHits) _relicTags.push('連殺');
      if (d.traumaProc) _relicTags.push('創傷');
      if (d.firePrisonMult) _relicTags.push('火焰牢獄');
      if (d.hardskinFireProc) _relicTags.push('硬皮引燃');
      if (d.autocastBacklash) _relicTags.push('施法反噬');
      if (d.spellIgnoreMr) _relicTags.push('無視魔防');
      if (d.critFuryHaste) _relicTags.push('暴怒加速');
      if (d.comboForceCrit) _relicTags.push('連擊必爆');
      if (d.drPerEr) _relicTags.push('閃避轉DR');
      if (d.freeChill) _relicTags.push('寒霜吐息');
      if (d.statArray) _relicTags.push('能力值陣列');
      if (d.polyAllStats) _relicTags.push('變身全屬性');
      if (d.grantSkills) _relicTags.push('賜予技能');
      if (d.wishRing) _relicTags.push('許願');
      if (d.stoneInstakill) _relicTags.push('石化斬殺');
      if (d.undeadImmune) _relicTags.push('亡靈庇護');
      if (d.golemMarkDebuff) _relicTags.push('巨人印記');
      if (d.liningChain) _relicTags.push('鏈甲內襯');
      if (d.stoneEssence) _relicTags.push('石化精華');
      if (d.playerHardSkin) _relicTags.push('玩家硬皮');
      if (d.petReviveBuff) _relicTags.push('寵物復活');
      if (d.allLures) _relicTags.push('全誘餌');
      if (d.raceDr) _relicTags.push('種族減傷');
      if (d.mpOnHit) _relicTags.push('命中回魔');
      if (d.hpOnHit) _relicTags.push('命中回血');
  ...
  function renderEquip(cls, slot) {
    ...
    function card(e) {
      var d = e.d, id = e.id;
      var nameCls = d.legend ? 'c-legend' : (d.relic ? 'c-relic' : 'text-slate-100');
  ...
  function renderPledge() {
    ...
    // 🐉 侵蝕安塔瑞斯巢穴
    out += wCard('🐉 侵蝕安塔瑞斯巢穴（每日副本）',
      wTbl(['項目', '內容'], [
        ['入場條件', '40 級以上；每角色每日限入 <b>1 次</b>（跨角色獨立計算）'],
        ['組隊', '隊員需在同一區才能一起打；可邀請其他玩家「助戰」（每人每天限助戰 1 次）'],
        ['副本結構', '第 1→2→3 區（小怪）→ 第 4 區「深處」打 BOSS 安塔瑞斯'],
        ['BOSS 三階段', '被侵蝕 → 狂怒 → 瘋狂（HP×2 翻倍、技能加強、transformHpPct=0.01 即 1% HP 觸發變身）'],
        ['死亡', '死亡會被傳回巢穴入口（不回城）；巢穴內不能回城'],
        ['獎勵', '安塔瑞斯掉「安塔瑞斯系列裝備」（含多魯嘉貝爾貝爾、多魯嘉貝爾貝爾等）']
      ]) +
      wDesc('每角色獨立計算次數，<b>多開角色可各打 1 次</b>。副本地圖見「地圖」分頁。'));

    // 🤝 NPC 私聊與外交系統
    out += wCard('🤝 NPC 私聊與外交系統',
      wDesc('NPC 有自己的<b>情感系統</b>（情緒值 -10 ～ +10）與<b>仇恨值</b>（0 ～ 100），影響與玩家的互動。') +
      wTbl(['機制', '說明'], [
        ['情感（情緒）', '-10～+10；低於 -5 時 NPC 可能攻擊玩家，高於 +5 時提供額外對話/服務'],
        ['仇恨', '0～100；累積仇恨會觸發 NPC 攻擊；透過贈禮（bond gifts）可降低仇恨'],
        ['私聊', '與 NPC 對話時可選擇「私聊」選項，觸發專屬對話與外交互動'],
        ['外交（diplomacy）', '盟主可透過 NPC 私聊進行「和平」或「挑釁」外交操作，影響 NPC 陣營態度'],
        ['個性化回應', '每個 NPC 有獨特的人格設定（persona），私聊回應依人格不同而異']
      ]) +
      wDesc('NPC 私聊系統在 26-world-channel.js 中實作，包含情緒/仇恨/贈禮/外交等完整互動。'));

    // 📊 Alignment 對齊值系統
    out += wCard('📊 Alignment 對齊值系統',
      wDesc('玩家有「對齊值」（alignment），影響傭兵的行為與陣營歸屬。') +
      wTbl(['項目', '內容'], [
        ['對齊值範圍', '-100 ～ +100（-100 = 完全敵對、0 = 中立、+100 = 完全友善）'],
        ['玩家對齊', '透過击杀敵對血盟成員、完成任務等方式改變'],
        ['傭兵對齊同步', '玩家對齊改變時，所有在場的傭兵會同步調整對齊值（alignmentDelta）'],
        ['傭兵對齊結算', '傭兵離場時，對齊差異透過 ledger 系統結算（與經驗值結算類似）'],
        ['對齊值重置', '角色死亡或特定事件可能重置對齊值'],
        ['影響', '對齊值影響 NPC 對你的態度、傭兵的忠誠度、以及某些任務的觸發條件']
      ]) +
      wDesc('對齊值機制在 06-status-allies.js 中實作，與傭兵系統深度整合。'));
