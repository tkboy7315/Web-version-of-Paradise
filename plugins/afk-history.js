 ...
 * 掛接:在 index.html 的 </body> 前加一行 <script src="plugins/afk-history.js"></script>
 * ========================================================================== */
(function () {
  'use strict';
  ...
  function renderToolbar() {
    ...
    if (slotFilter !== 'all' && !slots.some(function (s) { return s.slot === slotFilter; })) slotFilter = 'all';
  ...
  function closeModal() {
    var m = document.getElementById('m-hist-modal'); if (m) m.classList.remove('open');
  }
  ...
  function injectMainMenuBtn(menu) {
    if (document.getElementById('m-hist-open')) return;
    var b = document.createElement('button');
    b.id = 'm-hist-open';
    b.type = 'button';
    b.className = 'btn text-base w-72 py-2.5 bg-slate-700 hover:bg-slate-600 border-slate-500';
    b.textContent = '📜 離線掛機紀錄';
  ...
  function init() {
    ...
    injectCSS();
    buildModal();
    injectMainMenuBtn(menu);
    console.log('[AFK-history] hooks OK — 離線掛機紀錄已加入首頁選單。');
  }
