 ...
 *
 * 掛接方式:monkey-patch 包住 saveGame / loadGame / changeMap / killMob / gainItem。
 *   loadGame wrapper: 原 loadGame 前擷取離線錨點 → 原 loadGame 後呼叫 maybeCatchup。
 *   saveGame / changeMap wrapper: 原函式執行後呼叫 stamp()。
 *   killMob / gainItem wrapper: 結算期間依怪名/物品 id 累計計數。
 * ========================================================================== */
(function () {
  'use strict';
  ...
  async function runCatchup(totalTicks, withOverlay, huntMap, prePride, preObl, timing) {
    ...
    var FAST_MIN_HP_PCT = 60;         // 血量安全門檻起點 %(取樣用):真模擬 done=0 時的門檻
    var HP_FLOOR_ZERO_TICKS = 12000;  // 血量門檻「線性降到 0」的時點:真模擬連續存活滿 20 分鐘(12000 拍)沒死 → 門檻歸 0(之後一律切快速)。撐過這段=打得過→完全信任;死了外層撞死即停,根本走不到門檻歸 0。
    var BOSS_SAFE_MIN_HP = 0.2;       // 🐲 BOSS 專用安全線(獨立於取樣門檻):真打打贏、且血量沒掉到剩 20% 以下 → 記 safe,之後同名 BOSS 秒殺。
                                      //   原本跟 hpFloorNow() 共用取樣門檻(前段高達 70%),打 BOSS 血掉超過 30% 就被判「不安全」→ 每次都真打,
                                      //   整晚 BOSS 密集圖等於全程逐拍真模擬、結算龜速(2026-07-11 修)。險勝(掉到 20% 以下)才保留真打;
                                      //   打輸=真打時就撞死即停,根本不會記成 safe,安全度不因放寬下降。
    ...
    var BOSS_REVERIFY_P = 0.01;   // 🐲 抽驗:每 ~100 隻「已驗證安全」的同名 BOSS 抽 1 隻真打,實測耗時/同場小怪數做移動平均——單一首打樣本變異極大(同隻 BOSS 兩輪量到 27 vs 316 拍),外推整晚會嚴重失真。
                                  //   2026-07-11 由 0.05 降到 0.01:BOSS 密集圖一晚遇幾千隻 BOSS,5% 抽驗=幾百次逐拍真打(結算龜速主因);
                                  //   首打樣本已給基線,抽驗只需低頻覆核移動平均。
    ...
    function doCheckpoint() {
      try {
        if (typeof saveGame === 'function') saveGame();   // ff 下 logSys 靜音,不會洗「進度已儲存」;saveGame 尾端的 stamp 被 catchingUp 擋掉,不影響錨點
          ...
          if (fastMode) {
            ...
            if (fastBossUid != null) {   // 🐲 BOSS 對打中:逐拍真模擬(死亡由外層撞死即停接手;打不動就照實耗完時間)
              ...
              if (!_bAlive) {   // BOSS 倒下(或場面被重置)→ 記錄實測耗時/安全度,回快速段
                ...
                var _safeB = fastBossMinHp >= BOSS_SAFE_MIN_HP;   // 🐲 BOSS 專用安全線(血沒掉到 20% 以下即信任;險勝才保留真打) — 見 BOSS_SAFE_MIN_HP 註解
  ...
  function maybeCatchup(preMap, preTs, prePride, preObl) {
    ...
    var last = preTs;
    var savedMap = preMap;
    // 後援:舊資料沒有 afk_map → 從存檔 blob 補讀所在地圖。
    //   ⚠ 在 maybeCatchup 補讀是可以的(loadGame wrapper 在原 loadGame 之前已擷取 preMap,
    //   但若 preMap 為空(afk_map 尚未記錄),則退回讀存檔 blob——此時原 loadGame 已執行完,
    //   回村甦醒的存檔已蓋掉 blob,但在「剛裝外掛/舊資料」場景下,loadGame 還沒觸發 changeMap
    //   (maybeCatchup 在 loadGame wrapper 的 _load 之後才被呼叫),blob 可能還是正確的)。
    if (!savedMap) {
      try {
        ...
        try { var raw = JSON.parse(localStorage.getItem('lineage_idle_save_' + currentSlot)); savedMap = (raw && raw.ms && raw.ms.current) || ''; } catch (e2) {}
      }
    }
  ...
  // ----- 📜⚡ 擊殺/獲得計數:經 window.__afkKillTally/__afkGainTally(平時 null → 零開銷) -------
  window.__afkKillTally = null;
  window.__afkGainTally = null;

  // ----- 包裹 saveGame / loadGame / changeMap / killMob / gainItem (monkey-patch) ---------------

  // 包住 saveGame: 原 saveGame 執行後呼叫 stamp()(但 stamp() 在 catchingUp 期間跳過,不影響錨點)
  ...
  // 包住 loadGame: 必須在原 loadGame 之前擷取離線錨點(它會「在村莊甦醒」呼叫 changeMap → 被攔截 stamp() 覆寫 afk_map/afk_ts/afk_pride)
  //   → 原 loadGame 之後呼叫 maybeCatchup 決定要不要結算
  ...
  // 包住 changeMap:切地圖的「當下」就立即記錄即時地圖(+時間戳)。
  ...
  // ----- 除錯介面 ----------------------------------------------------------
  window.__afk = {
    version: '2.0.0',   // 2.x=外掛版 monkey-patch(afk-offline.js);1.x=舊外掛版
    capHours: CAP_HOURS,
    stamp: stamp,
    readTs: readTs,
  ...
  console.log('[AFK] hooks OK — 離線掛機外掛已啟用(上限 ' + CAP_HOURS + ' 小時，撞死即停，存活回原狩獵圖，分段檢查點防中斷)。');
})();
